...
export default function ThemeToggle(props) {
  // Using the custom hook to access the theme-related values
  ...
  const onChangeBtn = (e) => {
    ...
    // Call the appropriate theme function based on the checkbox status
    if (darkModeStatus) {
      darkTheme();
    } else {
      lightTheme();
  ...
  return (
    <button
      ...
      className={`cursor-pointer w-8 dsh-btn-icon rounded ${props.className}`}
    >
      <svg className="fill-current p-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        <path d={themeMode === "dark" ? SunPath : MoonPath}></path>
      </svg>
    </button>
  );
}