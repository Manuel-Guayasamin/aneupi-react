...
const ModalActualizarBiblioteca = ({
  showModal,
  closeModal,
  bibliotecaSelected,
}) => {
  ...
  const onSubmit = async (bibliotecaData) => {
    try {
      dispatch(
        updateBiblioteca({ id: bibliotecaSelected.id, bibliotecaData })
      ).then(() => {
  ...
  useEffect(() => {
    ...
    if (bibliotecaSelected) {
      Object.keys(bibliotecaSelected).forEach((key) => {
        setValue(key, bibliotecaSelected[key]);
      });
    }
  }, [setValue, bibliotecaSelected]);
        ...
        <div className="w-full max-w-lg bg-white rounded-lg shadow">
          <div className="p-6 space-y-4">
            <h1 className="text-xl font-bold leading-tight text-center text-gray-900">
              Actualizar Estado de la Biblioteca
            </h1>
            <form
...
export default ModalActualizarBiblioteca;
